...
<form action="<?php echo str_replace('&', '&amp;', $LPAY_TXN_URL); ?>" method="post" id="checkout">
	<div class="hiddenFormLPAYFieldsDiv">
  	...
  	<?php
  		if(isset($LPAY_PROMOCODE_STATUS) && isset($LPAY_promocode_LOCAL_VALIDATION) && isset($LPAY_PROMOCODE_VALUE)){
  			if($LPAY_PROMOCODE_STATUS=='enabled'){
  				if(($LPAY_promocode_LOCAL_VALIDATION=='enabled' && trim($LPAY_PROMOCODE_VALUE)!='') || $LPAY_promocode_LOCAL_VALIDATION=='disabled'){
  	?>
	  	<div class="form-group">
	  		...
	  		<div class="col-md-8" style="margin-bottom: 5px;">
				<span class="input-group-btn">
	  				<input type="text" id="promoCode" class="form-control pull-left" placeholder="LPAY Promo Code" style="width: 70%;">
	...
	$(document).ready(function(){
		$('.btnPromoCode').click(function(){
			...
			if($.trim($("#promoCode").val())!=''){
				...
				$.ajax({
					url: 'index.php?rt=extension/Lpay/applyCode',
					type: 'post',
					dataType: 'json',
					...
					success: function(res){
						...
						if(res.hiddenFields.length > 0){
							$('.hiddenFormLPAYFieldsDiv').html(res.hiddenFields);		
						}
					}
				});
